...
repressing_zwf = zeros(m,1); % To save the repressing level of zwf
repressing_pfk = zeros(m,1); % To save the repressing level of pfk
repressing_murAA = zeros(m,1); % To save the repressing level of murAA
UDPGlcNAc_yield = zeros(m,1); % To save the yield of UDPGlcNAc during m simulations
...
for i = 1:m
    ...
    zwf_RL = 0.97*rand(1); % relative repression level on zwf
    pfk_RL = 0.85*rand(1); % relative repression level on pfk
    murAA_RL = 0.7*rand(1); % relative repression level on murAA
    par.Km = 1.5*rand(1,v)+0.5.*ones(1,v);
    par.vmax = ones(1,v);
    par.n = 4*rand(1);
    par.ki =rand(1);
    par.vmax(1) = 0.5;
    par.vmax(5) = 0.4*(1-zwf_RL); % HMP 
    par.vmax(6) = 2*(1-pfk_RL); % EMP
    par.vmax(10) = 0.4*(1-murAA_RL); % PSP 
    par.vmax(11) = 0.4; 
    ...
    repressing_zwf(i,1) = zwf_RL;
    repressing_pfk(i,1) = pfk_RL;
    repressing_murAA(i,1) = murAA_RL;
    UDPGlcNAc_yield(i,1)=X(end,11);
    % The yield of UDPGlcNAc during m simulations
...
writematrix( [repressing_zwf repressing_pfk repressing_murAA UDPGlcNAc_yield],'yield.xlsx');
% create an Excel file, containing the yield of UDPGlcNAc during m simulations
end
% end
